...
def fetch_token_rows(conn, source_run_id):
    ...
    중요:
    youtube_comment_tfidf 테이블에는 parent_comment_id, comment_kind가 있으므로
    youtube_comment_token에서 이 값도 같이 가져와야 한다.

    조회 컬럼:
    - comment_id         : document ID
    - parent_comment_id  : 부모 댓글 ID. 원댓글이면 NULL, 대댓글이면 원댓글 ID
    - comment_kind       : TOP 또는 REPLY
    - token_norm         : 분석용 표준 token
    - pos                : 품사
...
def insert_tfidf_rows(conn, run_id, cfg, rows):
    ...
    comment_kind는 NOT NULL 컬럼이므로 반드시 INSERT에 포함한다.
    """
    if not rows:
        return 0
