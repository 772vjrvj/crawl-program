...
# CSV 저장
# =========================
def save_to_csv(items: List[Dict[str, Any]], csv_path: str = OUTPUT_CSV) -> None:
    if not items:
    ...
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        writer.writeheader()
        for item in items:
            row = {key: clean_text(item.get(key, "")) for key in fieldnames}
            writer.writerow(row)

    print(f"[OK] CSV 저장 완료: {csv_path}")
...
if __name__ == "__main__":
    ...
    save_to_csv(detail_url_list, OUTPUT_CSV)
